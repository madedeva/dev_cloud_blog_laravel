  ...
  <div class="card-body text-center">
    ...
    <p>
        <small class="text-muted">
            By. <a href="/authors/{{ $posts[0]->author->username }}" class="text-decoration-none text-primary">{{  $posts[0]->author->name }}</a> in <a href="/categories/{{  $posts[0]->category->slug }}" class="text-decoration-none text-primary">{{ $posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans() }}<p/>
        </small> 
    </p> 
        ...
        <div class="col-md-4 mb-3">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <p>
                        <small class="text-muted">
                            By. <a href="/authors/{{ $post->author->username }}" class="text-decoration-none text-primary">{{  $post->author->name }}</a> {{ $post->created_at->diffForHumans() }}<p/>
                        </small>
                    </p> 
